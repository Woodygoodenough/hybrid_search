...
class Histo2D:
    def __init__(self, token_bins: np.ndarray, revdate_bins: np.ndarray, counts: np.ndarray):
    ...
    @classmethod
    def from_records(cls, records: List[DBRecord], n_token: int = 10, n_revdate: int = 10) -> 'Histo2D':
        """Construct histogram from records with configurable dimensions."""
        # Extract data
        token_counts = [r.token_count for r in records]
        revdates = [datetime.strptime(r.revdate, '%Y-%m-%d %H:%M:%S').timestamp() for r in records]
    ...
    def rectangle_sum(self, token_range: Tuple[float, float], revdate_range: Tuple[float, float]) -> int:
        ...
        return int(np.sum(self.counts[token_start:token_end, revdate_start:revdate_end]))
    
    def __repr__(self) -> str:
        """String representation for Jupyter notebook display."""
        lines = [f"Histo2D({self.counts.shape[0]}x{self.counts.shape[1]})"]
        lines.append("Token bins: " + " ".join([f"{x:.0f}" for x in self.token_bins[:5]]) + 
                    ("..." if len(self.token_bins) > 5 else ""))
        lines.append("Revdate bins: " + " ".join([f"{datetime.fromtimestamp(x).strftime('%Y-%m-%d')}" for x in self.revdate_bins[:5]]) + 
    ...
    def draw_histo(self):
        ...
        plt.title('2D Histogram: Token Count Distribution by Revision Date', fontsize=14, fontweight='bold')
    ...
    def to_table(self) -> str:
        ...
        for i in range(self.counts.shape[1]):
            ...
            hi_dt = datetime.fromtimestamp(self.revdate_bins[i + 1]).strftime('%Y-%m-%d')
            if i == 0:
        ...
        for i in range(self.counts.shape[0]):
            ...
            row_data = [row_label] + [int(self.counts[i, j]) for j in range(self.counts.shape[1])]
    ...
    def revdate_edges_df(self):
        ...
        import pandas as pd
        formatted = [datetime.fromtimestamp(v).strftime('%Y-%m-%d %H:%M:%S') for v in self.revdate_bins]
        return pd.DataFrame({
    ...
    def display_summary(self) -> str:
        ...
        lines.append(f"Non-empty bins: {non_empty_bins}/{self.counts.shape[0] * self.counts.shape[1]} ({100*non_empty_bins/(self.counts.shape[0] * self.counts.shape[1]):.1f}%)")
        lines.append(f"Max count per bin: {max_count}")
        lines.append(f"Min count per bin: {min_count}")
        lines.append(f"Average count per bin: {total/(self.counts.shape[0] * self.counts.shape[1]):.2f}")
        lines.append(f"Average count per non-empty bin: {total/non_empty_bins:.2f}" if non_empty_bins > 0 else "Average count per non-empty bin: N/A")

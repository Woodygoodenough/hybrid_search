...
class Search:
    ...
    def simple_search(self, query: str, k: int, item_ids: Optional[List[int]] = None) -> Tuple[List[ArticleRecord], List[float]]:
        query_vec = self.embedder.encode_query(query)
        similarities, indices = self.index.search(query_vec, k, filter_ids=FilterIds.from_list(item_ids) if item_ids is not None else None)
    ...
    @staticmethod
    def search_results_to_df(records: List[ArticleRecord], similarities: List[float]) -> pd.DataFrame:
        df = pd.DataFrame(records, columns=["item_id", "title", "url", "revdate", "token_count", "entity"])
